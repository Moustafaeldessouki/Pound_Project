...
data = datasets.ImageFolder('face_classifier', transform=transform)
...
print(len(test_loader)*batch_size + len(valid_loader)*batch_size + len(train_loader)*batch_size)
...
for epoch in range(1, n_epochs + 1):
    ...
    if valid_loss <= valid_loss_min:
        print('Validation loss decreased ({:.6f} --> {:.6f}).  Saving model ...'.format(
            valid_loss_min,
            valid_loss))
        torch.save(model.state_dict(), 'model_pound.pt')
